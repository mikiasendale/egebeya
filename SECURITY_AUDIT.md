...
| **Real `CHAPA_WEBHOOK_SECRET` leaked into git history** | ❌ **CRITICAL-ish** | Value `***REMOVED***` matches `.env`; present in `.github/workflows/test.yml` and git history commits `28b13b5..f14d0ef` (via `_secrets_audit.mjs`). → Rotate now + purge history (BFG/filter-repo). |
