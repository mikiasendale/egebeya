...
import { tenants, users, media as mediaTable } from '../../src/db/schema';
...
describe('F7 — UUID-only media filenames', () => {
  ...
  afterAll(async () => {
    await db.delete(mediaTable).where(eq(mediaTable.tenantId, tenantId)).run();
  ...
  it('stored path is /uploads/<tenantId>/<uuid>.jpg (response may expose no originalName)', async () => {
    const res = await request(app)
      .post('/api/tenant/upload')
      .set('Authorization', 'Bearer ' + token)
      .attach('file', jpeg, { filename: 'leak.png', contentType: 'image/jpeg' });
    expect(res.status).toBe(200);
    const row = await db.select().from(mediaTable).where(eq(mediaTable.id, res.body.id)).get();
